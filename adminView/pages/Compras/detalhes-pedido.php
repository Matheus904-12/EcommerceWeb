...
$linkRastreio = null;
if (!empty($pedido['superfrete_label_id'])) {
    $linkRastreio = 'https://painel.superfrete.com.br/painel/track/' . urlencode($pedido['superfrete_label_id']);
    $codigoRastreio = $pedido['superfrete_label_id'];
    $nomeTransportadora = 'SuperFrete';
} elseif (!empty($pedido['tracking_code'])) {
    $linkRastreio = 'https://www.linkcorreios.com.br/?id=' . urlencode($pedido['tracking_code']);
...
<?php if ($linkRastreio): ?>
    <div class="flex items-center mb-4">
        <span class="font-mono text-lg bg-indigo-100 px-3 py-2 rounded text-indigo-800"><?php echo htmlspecialchars($codigoRastreio); ?> (<?php echo $nomeTransportadora; ?>)</span>
        <a href="<?php echo $linkRastreio; ?>" target="_blank" class="inline-block px-4 py-2 bg-indigo-500 text-white rounded-md shadow hover:bg-indigo-600 transition text-sm font-medium ml-2">
            <i class="fas fa-truck mr-2"></i> Rastrear Pedido
        </a>
    </div>
<?php endif; ?>
...
<body class="bg-indigo-50">
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white shadow-md rounded-lg p-6 mb-6">
            ...
            <?php if (isset($pedido['itens']) && is_array($pedido['itens']) && count($pedido['itens']) > 0): ?>
                                        ...
                                        <div class="flex items-center space-x-3">
                                            <?php if (!empty($item['produto_imagem'])): ?>
                                                <div class="flex-shrink-0 w-12 h-12">
                                                    <img class="w-12 h-12 object-cover rounded" src="../../uploads/produtos/<?= htmlspecialchars($item['produto_imagem']) ?>" alt="<?= htmlspecialchars($item['produto_nome'] ?? 'Produto') ?>">
                                                </div>
                                            <?php endif; ?>
                                            <div>
                                                <p class="text-indigo-800 font-medium"><?= htmlspecialchars($item['produto_nome'] ?? 'Produto') ?></p>
                                                <?php if (!empty($item['variacao'])): ?>
                                                    <span class="text-xs text-indigo-500">Variação: <?= htmlspecialchars($item['variacao']) ?></span>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4 text-indigo-700">
                                        <?= isset($item['quantidade']) ? (int)$item['quantidade'] : (isset($item['quantity']) ? (int)$item['quantity'] : 1) ?>
                                    </td>
                                    <td class="py-4 px-4 text-indigo-700">
                                        R$ <?= number_format(isset($item['produto_preco']) ? (float)$item['produto_preco'] : 0, 2, ',', '.') ?>
                                    </td>
                                    <td class="py-4 px-4 font-medium text-indigo-800">
                                        R$ <?= number_format((isset($item['produto_preco']) ? (float)$item['produto_preco'] : 0) * (isset($item['quantidade']) ? (int)$item['quantidade'] : (isset($item['quantity']) ? (int)$item['quantity'] : 1)), 2, ',', '.') ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
        ...
        <div class="mt-6 flex justify-end space-x-3">
            ...
            <button onclick="emitirNotaFiscal(<?= $orderId ?>)" class="px-4 py-2 bg-yellow-500 text-white rounded-md shadow hover:bg-yellow-600 transition">
    ...
    <script>
        // Função auxiliar para criar um atraso com Promise
        function delay(ms) {
        ...
        async function emitirNotaFiscal(orderId) {
            if (confirm(`Confirma a emissão da nota fiscal para o pedido #${orderId}?`)) {
                ...
                loadingModal.innerHTML = `
                    <div class="bg-white p-6 rounded-lg shadow-lg text-center">
                        ...
                        <p class="text-lg font-semibold">Emitindo nota fiscal...</p>
                ...
                try {
                    ...
                    const response = await fetch('../../controller/NotaFiscal/emitirNotaFiscal.php', {
                        method: 'POST',
                        body: new URLSearchParams({
                            order_id: orderId
                        }),
                    ...
                    if (data.success) {
                        // Criar modal de sucesso com detalhes da nota fiscal
                        const nfeInfo = data.nfe_info;
                        const successModal = document.createElement('div');
                        successModal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
                        successModal.innerHTML = `
                            <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                                <div class="flex justify-between items-center mb-4">
                                    <h3 class="text-lg font-semibold text-green-600">Nota Fiscal Emitida com Sucesso</h3>
                                    <button onclick="this.parentNode.parentNode.parentNode.remove()" class="text-gray-500 hover:text-gray-700">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </div>
                                <div class="mb-4">
                                    <p><strong>Número:</strong> ${nfeInfo.numero}</p>
                                    <p><strong>Série:</strong> ${nfeInfo.serie}</p>
                                    <p><strong>Status:</strong> ${nfeInfo.status || 'Processando'}</p>
                                </div>
                                <div class="flex justify-end space-x-2">
                                    ${nfeInfo.pdf_url ? `<a href="${nfeInfo.pdf_url}" target="_blank" class="px-4 py-2 bg-blue-500 text-white rounded-md shadow hover:bg-blue-600 transition">
                                        <i class="fas fa-file-pdf mr-2"></i> Visualizar PDF
                                    </a>` : ''}
                                    <button onclick="location.reload()" class="px-4 py-2 bg-gray-500 text-white rounded-md shadow hover:bg-gray-600 transition">
                        ...
                        alert("Erro ao emitir nota fiscal: " + (data.message || "Erro desconhecido"));
                    }
                } catch (error) {
                    // Remover indicador de carregamento
                    document.body.removeChild(loadingModal);
