...
function processPIXPayment($order_id, $amount, $user_id) {
    ...
    $customer_data = getUserData($user_id);
...
function processCreditCardPayment($order_id, $amount, $user_id, $card_data) {
    ...
    $payment_data = [
        ...
        'card_data' => [
            ...
            'cvv' => $card_data['cvv'],
            'save_card' => $card_data['save_card'] ?? false
        ],
        'installments' => $card_data['installments'] ?? 1,
        'customer' => [
            'document' => $card_data['document'] ?? '',
            'email' => $card_data['email'] ?? ''
        ]
    ];
    
    $payment_response = $gateway->processCreditCardPayment($payment_data);
...
function processCreditCardPaymentWithSavedCard($order_id, $amount, $user_id, $saved_card_id) {
    ...
    // Obter dados do cliente e do cartão
    $customer_data = getUserData($user_id);
    $card_data = getSavedCardData($saved_card_id);
    ...
    function getUserData($user_id) {
        $db = getDbConnection();
        $stmt = $db->prepare("SELECT name, email, document, phone FROM users WHERE id = ?");
        $stmt->execute([$user_id]);
        return $stmt->fetch(PDO::FETCH_ASSOC) ?: [];
    }
    
    /**
     * Função para obter dados do cartão salvo
     * @param int $card_id ID do cartão salvo
     * @return array Dados do cartão
     */
    function getSavedCardData($card_id) {
        $db = getDbConnection();
        $stmt = $db->prepare("SELECT card_token, last_four_digits, card_brand, card_holder_name FROM saved_cards WHERE id = ?");
    ...
    $payment_data = [
        ...
        'card_brand' => $card_data['card_brand'] ?? 'Visa',
        'installments' => $_POST['installments'] ?? 1,
        'customer_email' => $customer_data['email'] ?? '',
        'customer_document' => $customer_data['document'] ?? ''
    ];
